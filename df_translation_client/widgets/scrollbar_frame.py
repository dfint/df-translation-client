...
if __name__ == "__main__":
    ...
    scrollable_text = ScrollbarFrame(
        root, widget_factory=tk.Text, widget_args=dict(wrap=tk.NONE), scrollbar=ttk.Scrollbar
    )
    scrollable_text.pack()

    scrollable_listbox = ScrollbarFrame(
        root, widget_factory=tk.Listbox, scrollbar=ttk.Scrollbar, show_scrollbars=tk.VERTICAL
    )
